...
public class GUIControllerImpl implements Listener, ImageController {
  // change for hw 7: add a transformation command "mosaic" in the applyTransformationListen
  // method.
  ...
  public void applyTransformationListen(String transformation,
                                        String id, String newID, int increment, int seed) {
    switch (transformation) {
      ...
      case "blur":
        ...
        return;
      case "brighten":
        new BrightenCommand(increment).execute(model, id, newID);
        return;
      case "mosaic":
        new MosaicCommand(seed).execute(model, id, newID);
        return;
      default:
