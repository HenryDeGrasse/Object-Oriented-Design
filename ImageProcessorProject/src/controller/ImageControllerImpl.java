...
public class ImageControllerImpl implements ImageController {
  // change for hw 7: add mosaic as a new command object in beginRead method.
  ImageModel model;
  ImageView view;
  Readable readable;
  ...
  public void beginRead() {
    ...
    possibleCommands.put("mosaic", sc -> new MosaicCommand(sc.nextInt()));
    ...
    while (scan.hasNext()) {
      ...
      if (in.equalsIgnoreCase("load") || in.equalsIgnoreCase("save")) {
        ...
        /*
        try {
          view.renderImageRep(dest);
        } catch (IOException io) {
          throw new IllegalStateException("Failed to be read to readable");
        }
