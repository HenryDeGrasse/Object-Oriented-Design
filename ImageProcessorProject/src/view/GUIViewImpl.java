...
public class GUIViewImpl extends JFrame implements GUIView, ActionListener {
  // change for hw 7: add "mosaic" JMenuItem on the current GUIView and add its action listener.
  ...
  public GUIViewImpl(ImageModel imageModel) {
    ...
    ImageIcon i = new ImageIcon(new BufferedImage(
  ...
  public void actionPerformed(ActionEvent e) {
    switch (e.getActionCommand()) {
      ...
      case "blur":
        ...
        this.renderMessage("Transformation " + e.getActionCommand() + " on\n" + oldID
                + ".\nResults in image: " + currentID);
        this.renderImageRep(currentID);
        addToImageMap(currentID);
        visualizeHistogram("red", Color.red);
        break;
  ...
  protected void emitTransformationEvent(String transformation, String id,
                                         String newID, int increment, int seed) {
    for (Listener l : this.listener) {
      l.applyTransformationListen(transformation, id, newID, increment, seed);
    }
  }
  ...
  private void transform(String actionCommand) {
    ...
    if (actionCommand.equals("brighten")) {
      ...
      String incrementEntered = opNew.showInputDialog("Enter brightness value.");
      increment = Integer.valueOf(incrementEntered);
      this.emitTransformationEvent(actionCommand, idCurrent, idNew, increment, 0);
    }

    if (actionCommand.equals("mosaic")) {
      JOptionPane opMosaic = new JOptionPane();
      String seedEntered = opNew.showInputDialog("Enter seed value.");
      seed = Integer.valueOf(seedEntered);
      this.emitTransformationEvent(actionCommand, idCurrent, idNew, 0, seed);
    }
  }
  ...
  private void visualizeHistogram(String type, Color color) {
    ...
    System.out.println("histogram updated");
  }
